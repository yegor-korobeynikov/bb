...
- Substantial UI state is client-local (localStorage `bb.*` keys: sidebar collapse/order/mode, split layout, fixed panel tabs, prompt drafts, root-compose project id, zen mode); a native app needs AsyncStorage/MMKV equivalents and must decide which of these are per-device vs. shared.
